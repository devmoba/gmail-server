    ...
    devmoba.datatables = devmoba.datatables || {
        ...
        enableIndividualColumnSearch: (tableSelector, columnSettings) => {
            ...
            tableHead.children('tr:eq(1)').children('th').each((col, element) => {
                ...
                if (currentColSettings.searchDisabled) {
                    $(element).html('');
                } else if (currentColSettings.enableDateRangeFilter) {
                    $(element).html(`<input type="date" placeholder="Date from" class="search_rdatefrom_${col} form-control" style="width: 45%; display: inline-block; padding: 3px 7px;" onchange="devmoba.datatables.searchHelper.onfocusHandler(this); devmoba.datatables.searchHelper.onKeyupHandler(this);"/><span>-</span><input type="date" placeholder="Date to" class="search_rdateto_${col} form-control" style="width: 45%; display: inline-block; padding: 3px 7px;" onchange="devmoba.datatables.searchHelper.onfocusHandler(this); devmoba.datatables.searchHelper.onKeyupHandler(this);" />`);
                } else if (currentColSettings.enableRangeFilter) {
                    $(element).html(`<input type="text" placeholder="Min" class="search_rmin_${col} form-control" style="width: 45%; display: inline-block; padding: 3px 7px;" onkeyup="devmoba.datatables.searchHelper.onKeyupHandler(this)" onfocus="devmoba.datatables.searchHelper.onfocusHandler(this)"/><span>-</span><input type="text" placeholder="Max" class="search_rmax_${col} form-control" style="width: 45%; display: inline-block; padding: 3px 7px;" onkeyup="devmoba.datatables.searchHelper.onKeyupHandler(this)" onfocus="devmoba.datatables.searchHelper.onfocusHandler(this)" />`);
                } else if (currentColSettings.options) {
                    let dropdownHtml = `<select class="form-control search_c_${col}" style="padding: 3px 7px;" onchange="devmoba.datatables.searchHelper.onfocusHandler(this); devmoba.datatables.searchHelper.onKeyupHandler(this);">`;
        ...
        searchHelper: {
            ...
            executeFilter: (el) => {
                ...
                } else if (searchType === 'rdatefrom' || searchType === 'rdateto') {
                    var dateRangeCondition = {
                        col: col
                    };

                    if (searchType === 'rdatefrom')
                        dateRangeCondition.dateFrom = el.value;
                    else if (searchType === 'rdateto')
                        dateRangeCondition.dateTo = el.value;

                    let condIdDateRange = devmoba.datatables.searchHelper.dateRangeConditions.findIndex(c => c.col === col);
                    if (condIdDateRange > -1) {
                        if (dateRangeCondition.hasOwnProperty('dateFrom'))
                            devmoba.datatables.searchHelper.dateRangeConditions[condIdDateRange].dateFrom = dateRangeCondition.dateFrom;
                        if (dateRangeCondition.hasOwnProperty('dateTo'))
                            devmoba.datatables.searchHelper.dateRangeConditions[condIdDateRange].dateTo = dateRangeCondition.dateTo;
                        if (!devmoba.datatables.searchHelper.dateRangeConditions[condIdDateRange].dateFrom && !devmoba.datatables.searchHelper.dateRangeConditions[condIdDateRange].dateTo)
                            devmoba.datatables.searchHelper.dateRangeConditions.splice(condIdDateRange, 1);
                    } else {
                        devmoba.datatables.searchHelper.dateRangeConditions.push(dateRangeCondition);
                    }
                } else {
                    var rangeCondition = {
                        col: col
            ...
            getSearchConditions: () => {
                ...
                if (table.page.info() && table.page.info().serverSide === true) {
                    ...
                    let dateRangeConditions = devmoba.datatables.searchHelper.dateRangeConditions;
                    ...
                    devmoba.datatables.columnSettings.forEach((colSetting, colIndex) => {
                        ...
                        if (rangeSearchCond != null) { // if range search
                        ...
                        } else { // if not range search
                            rs[colName] = table.column(colIndex).search();
                        }

                        let dateRangeSearchCond = dateRangeConditions.find(cond => cond.col === colIndex);
                        if (dateRangeSearchCond != null) {
                            rs[`${colName}From`] = dateRangeSearchCond.dateFrom;
                            rs[`${colName}To`] = dateRangeSearchCond.dateTo;
                        }
                    });
                }
                return rs;
        ...
        fixDomConfiguration: (config) => {
            config.dom = '<"col-auto"l>rt<"row dataTable_footer"<"col-auto"i><"col"p>>';
            if (!config.scrollY)
                config.scrollY = "57vh";
            return config;
        }
